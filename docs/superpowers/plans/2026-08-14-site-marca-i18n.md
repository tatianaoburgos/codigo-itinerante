...
7. **Duplicação desnecessária entre as 9 páginas** (não prevista no design original — nasceu da leitura literal da seção 4 da spec, "cada uma só monta os mesmos componentes de sempre"): `index.astro`/`en/index.astro`/`es/index.astro` repetiam os mesmos 9 imports + 9 tags; `faq.astro` e `404.astro` repetiam o corpo inteiro em cada idioma. O próprio `template/` já resolve exatamente isso com um componente `PaginaInicial.astro` compartilhado (import único, corpo em um só lugar). Extraído `PaginaInicial.astro`, `PaginaFaq.astro` e `Pagina404.astro` (cada um resolve seu próprio idioma via `idiomaAtual(Astro.currentLocale)`, mesmo padrão dos outros componentes) — as 9 páginas viraram wrappers finos (`Layout` + 1 componente), com a única duplicação restante sendo o cálculo de `titulo`/`descricao`/`jsonLdFaq` por página (inevitável: só quem chama `<Layout>` pode passar props pra ele). Regressão zero confirmada via `Select-String` e checagem visual no navegador antes e depois do refactor.
